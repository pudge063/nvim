...
- **Номера строк** — по умолчанию абсолютные; `<leader>ur` переключает
  текущее окно на относительные (удобно для `5j`/`3dd` и т.п.) и обратно.
- **Конец строки** отмечается значком `↵` (`list`/`listchars`) — видно, где
  реально заканчивается строка, а не где её обрезал перенос при `wrap`.
